 ...
 * 中间节点指针指向自身或者自身前驱
 */
function selfBefore(nowData,beforeData){
	...
	for(var i=0;i<beforeData.length;i++){		
		...
		if(i==search_node(beforeData,action.changeId).y){
			
			for(var j=search_node(beforeData,action.changeId).x+1;j<beforeData[i].length;j++){
	    ...
	    for(var j = 0; j < nowData[i].length; j++){
		   ...
		   if(nowData[i][j].nextId!=""){
				 ...
				 if(clearMaoHao(nowData[i][j].id)==action.changeId)  {
						...
						d2 = transform(endY1,endX1,endY2,endX2,"curve");	
